package com.contral.common.redis.template;
...
public class RedisRepository {
    ...
    public void setExpire(final byte[] key, final byte[] value, final long time) {
        redisTemplate.execute((RedisCallback<Long>) connection -> {
            ...
            log.debug("[redisTemplate redis]放入缓存 url:{} ======缓存时间{}秒", key, time);
            return 1L;
        });
    }
    ...
    public Boolean setString(final String key, final String value) {
        ...
        redisTemplate.execute((RedisCallback<Boolean>) connection -> {
            ...
            return connection.set(keys,values);
        });
        log.debug("[redisTemplate redis]取出 缓存  url:{} ", key);
        return true;
    }
     ...
     * @return the string
     */
    public String getStringHashValue(String key, String hashKey) {
        if (StringUtils.isBlank(key)) {
            return null;
        }
        Object resultStr = redisTemplate.execute((RedisCallback<Object>) connection -> {
            ...
            byte[] rawHashKey = serializer.serialize(hashKey);
            if (null == rawKey || null == rawHashKey) {
                return null;
            }
            byte[] values = connection.hGet(rawKey, rawHashKey);
            if (null == values) {
                return null;
            }
            return new String(values);
    ...
    /**
     * redis List数据结构 : 将一个或多个值(Str) value 插入到列表 key 的表头
     *
     * @param key      the key
     * @param strValue the value
     * @return the Long
     */
    public Long leftPushStr(String key, String strValue) {
        if (StringUtils.isBlank(key)) {
            return null;
        }
        Long result = redisTemplate.execute((RedisCallback<Long>) connection -> {
            RedisSerializer<String> serializer = getRedisSerializer();
            byte[] rawKey = serializer.serialize(key);
            byte[] value = serializer.serialize(strValue);
            if (null == value) {
                return null;
            }
            if (null == rawKey) {
            ...
            return connection.listCommands().lPush(rawKey, value);
        });
        log.debug("[redisTemplate redis]存入 缓存  url:{} ", key);
        return result;
     ...
     * @return the string
     */
    public String rightPopStr(String listKey) {
        if (StringUtils.isBlank(listKey)) {
            return null;
        }
        Object resultStr = redisTemplate.execute((RedisCallback<Object>) connection -> {
            RedisSerializer<String> serializer = getRedisSerializer();
            byte[] rawKey = serializer.serialize(listKey);
            byte[] value = null;
            if (null != rawKey) {
                value = connection.listCommands().rPop(rawKey);
            }
            if (null == value) {
                return null;
            }
        ...
        log.debug("[redisTemplate redis]取出 缓存  url:{} ", listKey);
        return (String) resultStr;
     ...
     * @param tu
     */
    public void setExpire(String key, long time, TimeUnit tu) {
